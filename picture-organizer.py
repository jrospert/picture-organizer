...
def filenameIncrementor(date_time, file_type, inc):
...
def imageAnalyzer(dir):
    ...
    for file in os.listdir(dir):
        ...
        if fileExists(filename, dir):
            ...
            incFilename = filenameIncrementor(dateTime, fileType, incrementor)            		
            ...
            while fileExists(incFilename, dir):
...
def main():
    #r is for raw string so '\' is not interpreted as an escape sequence
    os.chdir(r'C:\Users\Joel\Desktop\phone') 
    phonedir = r'C:\Users\Joel\Desktop\phone'
    print("phonedir is : " + phonedir)

    if not os.path.exists("fb"):
        os.makedirs("fb",711)
        fbdir = os.path.join(phonedir, 'fb')
    else:
...
def valid_args(arguments, *valid):
    ...
    for argument in arguments:
        if not argument[0] == None or argument[0] == False:
            args_provided.append(argument[0])
    for arg_provided in args_provided:
        if not args_provided in valid:
            return False
    return True
...
if __name__ == "__main__":
    ...
    parser.add_argument("--file_dir", help="location of photos to go through")
